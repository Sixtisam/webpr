...
crazySuite.add("equals", (assert) => {
  ...
  assert.is(a == b, true);
  assert.is(b == c, true);
  // "0" is converted to ['0'] and therefore not an empty array
  assert.is(a == c, false); // "0" != []
  // "\t" != "0"
  // [] != "\t"
...
crazySuite.add("other", (assert) => {
  ...
  assert.is([] == [], false);
  assert.is([] == ![], true); // coercion to boolean
  assert.is(+[], 0);
  assert.is(2 == [2], true);
  // {} + [] empty block, unary + operator === 0
  // ({} + []) object
...
crazySuite.add("numbers", (assert) => {
  ...
  assert.is(999999999999999999, 999999999999999992); // too high number rounding issue
  assert.is(Number.MAX_VALUE > 0, true);
  // Min Value is the closed positive number to 0 (not NEGATIVE)
  assert.is(Number.MIN_VALUE < 0, false);
  assert.is(Math.min(1, 2, 3) < Math.max(1, 2, 3), true);
  // Math.min() is Infinity whereas Math.max is -Infinity
